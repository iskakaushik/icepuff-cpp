...
class CompressionCodec {
 public:
  ...
  std::optional<std::string_view> codec_name() const {
    switch (value_) {
      case NONE:
        return std::nullopt;
      case LZ4:
  ...
  static Result<CompressionCodec> from_name(
    ...
    return Result<CompressionCodec>(ErrorCode::kUnknownCodec,
                                    "Unknown codec name");
  }

  // Comparison operators
  constexpr bool operator==(const CompressionCodec& other) const {
